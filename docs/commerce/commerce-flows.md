...
**גרסה 1.1** · עודכן בסבב התיקונים: תרשים 13 (ביטול — אין `cancelled` לפני זיכוי; מצב `cancel_pending_refund` בציר ההזמנה), תרשים 18 (מנגנון Claim בטוח), הערות A13/A14 (לכידה באריזה, מסמך באריזה) על תרשימים 8–9. פירוט ב[יומן השינויים](./commerce-changelog.md).

> חלק מחבילת אפיון המסחר: [מסמך האב](./commerce-master-spec.md) · [מודל הנתונים](./commerce-data-model.md) · [ניתוח פערים](./commerce-gap-analysis.md) · [תוכנית יישום](./commerce-implementation-plan.md) · [החלטות](./commerce-decisions.md) · [מפרט מסכים](./commerce-ui-spec.md)
>
...
stateDiagram-v2
    ...
    confirmed --> cancel_pending_refund: אושר ביטול על הזמנה ששולמה [1.1]
    processing --> cancel_pending_refund: ביטול חריג על הזמנה ששולמה (אישור כפול) [1.1]
    cancel_pending_refund --> cancelled: זיכוי מלא הצליח במורנינג [1.1]
    cancel_pending_refund --> confirmed: חזרה מהביטול (זיכוי לא בוצע, החלטת צוות) [1.1]
...
| →cancelled (מהזמנה שלא שולמה) | לקוח (רק מ־pending, ותנאי פרק 14); צוות; מערכת (פקיעה) | `cancelled` + סיבה |
| **[1.1]** confirmed/processing→cancel_pending_refund | צוות בהרשאת ביטול (על הזמנה ששולמה) | `cancel_approved` + סיבה |
| **[1.1]** cancel_pending_refund→cancelled | **מערכת בלבד** — אחרי `refund succeeded` בסכום המלא | `cancelled` + מזהה הזיכוי |
| →closed | מערכת (job) / צוות | `status_changed` |

**[1.1]** הזמנה ששולמה לעולם אינה עוברת ישירות ל־`cancelled`: המסלול היחיד הוא דרך `cancel_pending_refund`, והמעבר הסופי נרשם רק על־ידי המערכת אחרי שהזיכוי המלא הצליח (תרשים 13). זיכוי חלקי משאיר את ההזמנה ב־`confirmed`/`processing` עם `payment_state=partially_refunded`.
...
**[1.1] וריאנט "לכידה באריזה" (הנחה A13, פרק 8.3 במסמך האב):** אם ייאושש שמורנינג תומכת ב־Authorize בהזמנה + ‏Capture באריזה כולל עדכון סכום — תרשימים 7–8 מקבלים שלב ביניים: ‏Webhook האישור מסמן `payment_state=authorized` (שריון מסגרת בלבד, המלאי נשמר אך אינו נלכד כספית), ובעת האריזה השרת מעדכן את הסכום (תוספות ללא דמי משלוח חדשים / הפחתת פריט חסר) ומבצע ‏Capture — שרק הוא מעביר ל־`paid` ומפעיל את הפחתת המלאי הסופית ואת הפקת המסמך. עד להכרעת A13 — התרשימים כלשונם (חיוב מיידי) הם המחייבים.
...
**[1.1] תזמון המסמך (הנחה A14):** בקשת בעל האתר — המסמך (חשבונית/קבלה) יופק ויישלח **בשלב האריזה**, לא בעת התשלום. אם A14 תאושש: העסקה נוצרת בלי מסמך אוטומטי, `document_state` נשאר `pending` עד האריזה, וסימון "ההזמנה נארזה" מפעיל את ענף ההפקה היזומה שבתרשים (המייל "אישור תשלום" נשלח מיד, בלי המסמך; מייל המסמך נשלח מהאריזה). אם A14 תיכשל — נשארים בהפקה אוטומטית עם העסקה, והבקשה מסומנת כלא־ישימה בטבלת ההנחות.
...
## 13. ביטול הזמנה **[1.1 — תוקן: סעיף 9 בסבב התיקונים]**

**כלל הזהב:** הזמנה ששולמה אינה עוברת ל־`cancelled` לפני שזיכוי **מלא** הצליח בפועל במורנינג. כישלון או המתנה בזיכוי משאירים את ההזמנה במצב טיפול ייעודי — לא במצב מבוטל ולא במצב רגיל.
...
flowchart TD
    ...
    F -- אישור --> C
    C --> G{שולם?}
    G -- לא --> H[state=cancelled מיידי<br/>commerce_release_stock — שחרור שמירה]
    G -- כן --> I[מסך זיכוי — תרשים 15<br/>ההזמנה עוברת cancel_pending_refund]
    I --> R{תוצאת הזיכוי}
    R -- הצליח במלואו --> S[רק עכשיו: state=cancelled<br/>payment_state=refunded]
    R -- זיכוי חלקי בלבד --> P[ההזמנה נשארת פעילה —<br/>partially_refunded. ביטול מלא מחייב<br/>השלמת הזיכוי או החלטת צוות מתועדת]
    R -- נכשל / ממתין --> Q[נשאר cancel_pending_refund:<br/>תצוגת טיפול ייעודית + התראה,<br/>ניסיון חוזר ללא זיכוי כפול]
    Q --> I
    S --> J{סטטוס פיזי של הפריטים}
    J -- טרם נשלחו: נשארו במחסן --> K1[commerce_restock מיידי —<br/>תנועת cancel_restock, אין המתנה]
    J -- נשלחו וחזרו פיזית --> K2[החזרה למלאי מפורשת בלבד<br/>אחרי בדיקת מצב: תקין? לאיזה מיקום? — תרשים 14]
    J -- נשלחו וטרם חזרו --> K3[אין restock עד קבלה פיזית]
...
**ארבעת התיקונים המחייבים (1.1):**
1. **אין `cancelled` לפני זיכוי:** המעבר ל־`cancelled` בהזמנה ששולמה מותנה בזיכוי מלא שהצליח (`refund succeeded` בסכום המלא ששולם). מצב הביניים `cancel_pending_refund` (ציר ההזמנה — סעיף 1.1 לעיל) מייצג "אושר ביטול, ממתין לזיכוי".
2. **זיכוי חלקי לעולם אינו מבטל אוטומטית:** הזמנה עם `partially_refunded` נשארת בתוקף על יתרתה. ביטול מלא ממנה — רק בהשלמת הזיכוי.
3. **הפרדת restock:** פריטים שלא נשלחו חוזרים למלאי מיידית עם הביטול (תנועת `cancel_restock`); פריטים שנשלחו — רק אחרי חזרה פיזית ובדיקת מצב (תרשים 14). אין ענף שבו מלאי חוזר "על הנייר" בזמן שהסחורה אצל הלקוח.
4. **כשל זיכוי הוא מצב מנוהל:** `cancel_pending_refund` מופיע בתצוגה שמורה ייעודית עם התראה; ניסיון זיכוי חוזר ממחזר את אותה רשומת refund (‏idempotency) — אין זיכוי כפול.
...
sequenceDiagram
    ...
    U->>S: לחיצה על ההצעה — **עם טוקן ההזמנה** (256 ביט)
    Note over S: הטוקן מוכיח בעלות על הזמנת המקור —<br/>זה עוגן ה-Claim, לא הטלפון לבדו
    S->>A: יצירת/איתור משתמש (OTP לטלפון או מייל+סיסמה — בחירת הלקוח, פרק 4.4)
    ...
    S->>DB: שיוך הזמנת המקור בלבד (המוכחת בטוקן):<br/>orders.user_id = המשתמש החדש
    S->>DB: הזמנות עבר: שיוך רק בהתאמה כפולה —<br/>גם הטלפון וגם המייל זהים לערך המאומת
    Note over DB: התאמה חלקית (רק טלפון או רק מייל) ⇒<br/>אין שיוך אוטומטי. אי-ודאות = לא ממזגים
    S->>DB: consent_events: terms/channels כפי שסומנו ב-Checkout
    S-->>U: ׳החשבון מוכן׳ + כניסה לאזור האישי<br/>הזמנת המקור כבר מופיעה בו
```

**[1.1] מנגנון Claim בטוח (סעיף 7 בסבב התיקונים) — מחליף שיוך אוטומטי לפי טלפון:**

- **עוגן ה־Claim הוא טוקן ההזמנה,** לא הטלפון: אימות OTP לטלפון לבדו **אינו** מספיק לשיוך הזמנות עבר. טלפונים משותפים (משפחה, קו כשר משותף) הם תרחיש נפוץ בקהל היעד — מספר מאומת אינו מוכיח בעלות על כל ההזמנות שאי־פעם הוזן בהן.
- **הזמנת המקור** (שמטוקנה התחיל ה־Claim) משויכת תמיד — הטוקן מוכיח אותה.
- **הזמנות עבר** משויכות רק ב**התאמה כפולה**: גם הטלפון וגם המייל שבהזמנה זהים לערכים המאומתים של החשבון. התאמה חלקית ⇒ ללא שיוך, בלי הודעת שגיאה מפורטת (לא מדליפים אילו הזמנות קיימות).
- **בכל ספק — לא ממזגים.** הזמנה שלא שויכה נשארת נגישה בטוקן שלה; דבר אינו אובד.
- **נתיב ידני:** לקוח שטוען להזמנות חסרות פונה לצוות; הצוות מאמת (פרטי הזמנה, כתובת, סכום) ומשייך ידנית בהרשאה ייעודית + תיעוד ב־audit.

אין טופס נוסף מעבר לאימות; סיסמה — רק אם הלקוח בחר במסלול מייל+סיסמה (פרק 4.4).
