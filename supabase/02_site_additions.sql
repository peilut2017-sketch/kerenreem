...
-- ----------------------------------------------------------------------------
-- 3. audit_log — הרשאת כתיבה לצוות העריכה
-- ----------------------------------------------------------------------------
-- בסכימת הבסיס יש ל-audit_log מדיניות קריאה בלבד, מתוך הנחה שהתיעוד ייכתב
-- בצד השרת עם service_role. בפועל האתר כותב אותו עם ה-session של העורך,
-- כדי לא להחזיק מפתח שעוקף RLS בנתיב עריכה רגיל — ולכן דרושה מדיניות
-- הוספה. בלעדיה כל רישום נדחה ב-42501 והיומן נשאר ריק.
--
-- ההגבלה: עורך יכול לרשום רק שורות בשמו שלו. אין עדכון ואין מחיקה —
-- יומן ביקורת שניתן לערוך אינו יומן ביקורת.
