 ...
 *
 * ‏[1.42] העמודים הציבוריים **סטטיים** ומתרעננים on-demand בכל שמירה
 * דרך הטופס (ראו revalidateEntity למטה ו-entity.revalidate ב-schema.ts).
 * שלושה מהם עוד נושאים חלון זמן, וכל אחד מסיבה מפורשת שמתועדת בו:
 * ‏/events (פיצול קרובים/היו לפי היום), ‎/ ו-/books* (חלונות מבצע ובאנר,
 * ‎sale_starts_at/sale_ends_at ו-banners.starts_at/ends_at).
 *
 * שמירה שלא עברה דרך הטופס — למשל שורה שנוספה ישירות ב-SQL Editor —
 * אינה קוראת ל-revalidatePath בכלל, ואז המטמון אינו יודע שיש מה לרענן.
 * הפעולה הזו היא הדרך לכפות רענון בלי לגעת בקוד, וככל שיש פחות חלונות
 * זמן — כך היא נחוצה יותר.
 *
