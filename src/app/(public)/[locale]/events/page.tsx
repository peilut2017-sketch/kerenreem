...
/*
 * ‏[1.42] סטטי + on-demand בלבד.
 *
 * העמוד מפצל את הרשימה ל"אירועים קרובים" ול"אירועים שהיו", והפיצול
 * נעשה כאן בזמן הרינדור מול *היום* (‏isUpcoming, למטה). אירוע שמתקיים
 * היום עובר ל"היו" בחצות — בלי ששום שורה במסד משתנה.
 *
 * הגבול הזה מטופל עכשיו ב-/api/cron/revalidate יחד עם חלונות המבצע
 * והבאנרים: הוא מזהה אירוע שחצה את חצות ומרענן /events ו-/en/events
 * בלבד. ראו eventFlipAt ב-lib/revalidation/boundaries.ts, ששם מתועד גם
 * למה הרגע המדויק הוא תחילת היום *שאחרי* תאריך האירוע.
 */
