  ...
  const productLd = showBuy
    ? {
        ...
        offers: {
          ...
          /*
           * ‏[1.42] שני דברים שיש לדעת כאן, ושניהם **לא שונו** במכוון.
           *
           * ‏1. הערך הזה נצרב בעמוד הסטטי ואינו מתעדכן אחרי ה-hydration,
           *    בשונה מכפתור הקנייה (שעבר לזמינות חיה — ראו
           *    lib/books/availability-actions.ts). כלומר הוא יכול להיות
           *    מיושן עד לרענון הבא של העמוד. זו החלטה מפורשת: להחזיר
           *    ‏revalidation על כל תנועת מלאי רק בשביל גוגל היה מחזיר את
           *    כל ה-churn של reserve/release ל-ISR, והרווח אינו מצדיק —
           *    גוגל סורק את העמוד כל כמה ימים בלי קשר.
           *
           * ‏2. **ספק ממשי לגבי הערך עצמו, לעיון בהמשך.** ‏availability
           *    נגזר מ-books.stock_quantity, ש-30_inventory.sql מגדיר כ-
           *    ‏`sum(on_hand - reserved)` — כלומר הוא **מנוכה שריונות
           *    זמניים**. מכאן: ספר עם עותק אחד במחסן, שלקוח אחד פתח עליו
           *    ‏checkout ולא שילם, מפורסם לגוגל כ-OutOfStock — ולפי
           *    ‏expireStalePendingOrders(7) השריון הזה יכול להחזיק **שבעה
           *    ימים**. זה אינו "מיושן", זה שגוי: הספר כן למכירה.
           *
           *    לדעתי הערך הנכון ל-schema.org הוא "האם הספר נמכר בכלל",
           *    כלומר לפי on_hand (או is_purchasable + מחיר), ולא לפי הזמין
           *    נטו אחרי החזקות. הסמנטיקה של OutOfStock היא "אין לי את זה",
           *    לא "מישהו אחר אוחז בזה כרגע". רישום שגוי כזה מוריד את
           *    העמוד מ-Merchant Listing ופוגע בחשיפה.
           *
           *    לא שינוי כאן כי זה שינוי התנהגות שנוגע ל-SEO ודורש הכרעה
           *    נפרדת, לא שינוי אגב.
           */
          availability:
